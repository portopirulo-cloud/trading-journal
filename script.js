...
els.filterDate.addEventListener('change', () => { activeDateFilter = els.filterDate.value; render(); });
...
function render() {
  ...
  let list = trades;
  if (activeFilter) list = list.filter(t => t.asset === activeFilter);
  if (activeDateFilter) list = list.filter(t => t.date === activeDateFilter);
  ...
  els.emptyState.textContent = trades.length && !list.length
    ? 'No hay operaciones que coincidan con el filtro.'
    : 'Todavía no registraste operaciones. Cargá la primera arriba.';
...
function renderDaySummary() {
  if (!activeDateFilter) {
    els.daySummary.classList.remove('active');
    els.daySummary.innerHTML = '';
    return;
  }
  const dayTrades = trades.filter(t => t.date === activeDateFilter && (!activeFilter || t.asset === activeFilter));
  const lost = dayTrades.filter(t => pnlOf(t) < 0).reduce((s, t) => s + pnlOf(t), 0);
  const won = dayTrades.filter(t => pnlOf(t) > 0).reduce((s, t) => s + pnlOf(t), 0);
  const net = won + lost;
  els.daySummary.classList.add('active');
  if (!dayTrades.length) {
    els.daySummary.innerHTML = `Sin operaciones registradas el <strong>${activeDateFilter}</strong>.`;
    return;
  }
  els.daySummary.innerHTML = `
    <span><strong>${dayTrades.length}</strong> operación(es) el <strong>${activeDateFilter}</strong></span>
    <span>Perdido: <span class="loss">${fmtMoney(lost)}</span></span>
    <span>Ganado: <span class="win">+${fmtMoney(won)}</span></span>
    <span>Neto del día: <span class="net ${net > 0 ? 'pos' : net < 0 ? 'neg' : ''}">${net >= 0 ? '+' : ''}${fmtMoney(net)}</span></span>`;
}
